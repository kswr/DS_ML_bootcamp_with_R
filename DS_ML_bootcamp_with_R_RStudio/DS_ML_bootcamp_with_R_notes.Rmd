...
1. Facets (pl. płaszczyzny) (optional)
...
Plotly is foss library for creating interactive graphs and visualisations in JS, R, Python, Scala and Matlab

`ggplotly()` is `plotly` function wchich converts `ggplot2` script into plotly.js files which plotly is based on
...
batting <- read.csv('../R-for-Data-Science-and-Machine-Learning/Training Exercises/Capstone and Data Viz Projects/Capstone Project/Batting.csv')

# check content
head(batting)
str(batting)

# head of the first five rows of AB column
head(batting$AB, n=5L)
...
# load salaries data
salaries <- read.csv('../R-for-Data-Science-and-Machine-Learning/Training Exercises/Capstone and Data Viz Projects/Capstone Project/Salaries.csv')

# subset batting data for 1985 and onwards
sub_batting <- subset(batting, yearID >= 1985)

summary(sub_batting)

# merge batting and salary data

merged <- merge(sub_batting, salaries, by = c('playerID', 'yearID'))

summary(merged)

# get data about lost players

boys <- subset(merged, playerID %in% c('giambja01','damonjo01','saenzol01'))

boys2001 <- subset(boys, yearID == 2001)

boys2001 <- select(boys2001, playerID, H, X2B, X3B, HR, OBP, SLG, BA, AB)

# find replacement players (solution is manual, not much of data science)

avail.players <- subset(merged, yearID == 2001)
...
Introduction to Statistical Learning by Gareth James will be used in this section as a companion book (remember to print)
